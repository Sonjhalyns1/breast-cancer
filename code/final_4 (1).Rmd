...
```{r}
bc = read.csv("/Users/sonjhalynsaugustin/Downloads/BreastCancer.csv")
library(addAlpha)
...
```
```{r the (or many) bar graph(s), fig.align='center', fig.height = 6, fig.width=12,  fig.cap=""}
thePredictorVariables <- c("area_mean","radius_mean", "concave.points_mean", "smoothness_mean", "compactness_mean", "concavity_mean", "texture_mean", "symmetry_mean", "fractal_dimension_mean", "perimeter_mean")
thePredictorVariablesPretty <- c("Area\nMean", "Radius\nMean", "Concave\nPoints\nMean", "Smoothness\nMean", "Compactness\nMean", "Concavity\nMean", "Texture\nMean", "Symmetry\nMean", "Fractal\nDimension\nMean", "Perimeter\nMean")

par(mfcol = c(1,length(thePredictorVariables)), mai = c(.1,.2,.1,.2), omi = c(.2,.2,.5,.2))

for (i in 1:length(thePredictorVariables)){

  currentData <- mean_bc[, thePredictorVariables[i]]

  if (thePredictorVariables[i] == "radius_mean") {
    yLim <- c(5, 30)
    }else if (thePredictorVariables[i] == "texture_mean") {
    yLim <- c(5, 40)
    }else if (i != 1 & i < length(thePredictorVariables)){
    yLim <- range(pretty(range(log(currentData+1))))
  } else {
    yLim <- range(pretty(range(currentData)))
  }

  boxplot(currentData ~ mean_bc$diagnosis, ylim = yLim, axes = FALSE, ann = FALSE, col = "white", 
            border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25)

  axis(2, at = pretty(yLim), lwd = 2, tck = -.02, labels = FALSE)
  mtext(pretty(yLim), 2, at = pretty(yLim), line = .3, las = 1)
  mtext(thePredictorVariablesPretty[i], 3, font = 2)
}
...
# Hist and boxplots
```{r}
thePredictorVariables <- c("area_mean","radius_mean", "concave.points_mean", "perimeter_mean", "smoothness_mean", "compactness_mean", "concavity_mean", "texture_mean", "symmetry_mean", "fractal_dimension_mean")
thePredictorVariablesPretty <- c("Area\nMean", "Radius\nMean", "Concave\nPoints\nMean", "Perimeter\nMean", "Smoothness\nMean", "Compactness\nMean", "Concavity\nMean", "Texture\nMean", "Symmetry\nMean", "Fractal\nDimension\nMean")

#---------------CONCAVITY MEAN----------------------


  myMatrix <- matrix(c(1,2), nrow = 2, ncol = 1)
    layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
    par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
  xyLim <- range(pretty(range(mean_bc$concavity_mean)))
  benign <- mean_bc[mean_bc$diagnosis == 0, "concavity_mean"]
  hist_benign <- hist(benign,   plot = FALSE)
  
  # Calculate histogram for malignant cases
  malignant <- mean_bc[mean_bc$diagnosis == 1, "concavity_mean"]
  hist_malignant <- hist(malignant, plot = FALSE)
  
  # Set y-axis limits
  yLim <- range(pretty(range(hist_benign$counts, hist_malignant$counts)))
  
  # Calculate bin width
  dataRange <- range(mean_bc$concavity_mean)
  binWidth <- (max(dataRange) - min(dataRange)) / 9
  theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
  
  # Plot histogram for benign cases
  hist(benign, breaks = theBreaks, ylim = yLim, axes = FALSE, col = colorsT[4], ann = FALSE, border = colorsT[4])
  # Add histogram for malignant cases on top of benign cases
  hist(malignant, breaks = theBreaks, add = TRUE,  col = colorsT[3], border = colorsT[3])
  # Add y-axis with tick marks
  axis(2, at = pretty(yLim), labels = FALSE, tck = -.015, lwd = 2)
  mtext(pretty(yLim), line = .4, side = 2, las = 1, at = pretty(yLim))
  mtext("Frequency", 2, font = 2, line = 2)
  
  legend("topright", legend = c("benign","malignant"), fill = c(colorsT[4],colorsT[3]), border = colors[c(4,3)], bty= "n")
  
  xLim <- range(pretty(range(mean_bc$concavity_mean)))
  boxplot(concavity_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
  
  
  axis(side = 1, at = pretty(xLim), labels = FALSE, tck = -.015, lwd = 2)
  mtext(pretty(xLim), line = .4 , side = 1, las = 1, at = pretty(xLim))
...
layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
xyLim <- range(pretty(range(mean_bc$radius_mean)))
benign <- mean_bc[mean_bc$diagnosis == 0, "radius_mean"]
hist_benign <- hist(benign, plot = FALSE)

# Calculate histogram for malignant cases
malignant <- mean_bc[mean_bc$diagnosis == 1, "radius_mean"]
hist_malignant <- hist(malignant, plot = FALSE)

# Set y-axis limits
yLim <- range(pretty(range(hist_benign$counts, hist_malignant$counts)))
yLim <- c(yLim[1], yLim[2] * 2) # Increase y-axis limit by 20%

# Calculate bin width
dataRange <- range(mean_bc$radius_mean)
binWidth <- (max(dataRange) - min(dataRange)) / 9
theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
...
xLim <- range(pretty(range(mean_bc$radius_mean)))
boxplot(radius_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
...
#-------------------------------------------------------------CONCAVE POINTS MEAN------------------------------------------------------------------
  ...
  myMatrix <- matrix(c(1,2), nrow = 2, ncol = 1)
    layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
    par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
  xyLim <- range(pretty(range(mean_bc$concave.points_mean)))
  benign <- mean_bc[mean_bc$diagnosis == 0, "concave.points_mean"]
  hist_benign <- hist(benign, plot = FALSE)
  
  # Calculate histogram for malignant cases
  malignant <- mean_bc[mean_bc$diagnosis == 1, "concave.points_mean"]
  hist_malignant <- hist(malignant, plot = FALSE)
  
  # Set y-axis limits
  yLim <- range(pretty(range(hist_benign$counts, hist_malignant$counts)))
  yLim <- c(yLim[1], yLim[2] * 1.65)
  # Calculate bin width
  dataRange <- range(mean_bc$concave.points_mean)
  binWidth <- (max(dataRange) - min(dataRange)) / 9
  theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
  ...
  xLim <- range(pretty(range(mean_bc$concave.points_mean)))
  boxplot(concave.points_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
...
#----------------------------------------------------------------AREA MEAN-----------------------------------------------------------------------------
  ...
  myMatrix <- matrix(c(1,2), nrow = 2, ncol = 1)
    layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
    par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
  xyLim <- range(pretty(range(mean_bc$area_mean)))
  benign <- mean_bc[mean_bc$diagnosis == 0, "area_mean"]
  hist_benign <- hist(benign, plot = FALSE)
  
  # Calculate histogram for malignant cases
  malignant <- mean_bc[mean_bc$diagnosis == 1, "area_mean"]
  hist_malignant <- hist(malignant, plot = FALSE)
  
  # Set y-axis limits
  yLim <- range(pretty(range(hist_benign$counts, hist_malignant$counts)))
  yLim <- c(yLim[1], yLim[2] * 2.05)
  # Calculate bin width
  dataRange <- range(mean_bc$area_mean)
  binWidth <- (max(dataRange) - min(dataRange)) / 9
  theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
  ...
  xLim <- range(pretty(range(mean_bc$area_mean)))
  boxplot(area_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
...
#----------------------------------------------------------------PERIMETER MEAN-----------------------------------------------------------------------------  
  ...
  myMatrix <- matrix(c(1,2), nrow = 2, ncol = 1)
    layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
    par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
  xyLim <- range(pretty(range(mean_bc$perimeter_mean)))
  benign <- mean_bc[mean_bc$diagnosis == 0, "perimeter_mean"]
  hist_benign <- hist(benign, plot = FALSE)
  
  # Calculate histogram for malignant cases
  malignant <- mean_bc[mean_bc$diagnosis == 1, "perimeter_mean"]
  hist_malignant <- hist(malignant, plot = FALSE)
  
  # Set y-axis limits
  yLim <- range(pretty(range(hist_benign$counts, hist_malignant$counts)))
  yLim <- c(yLim[1], yLim[2] * 1.4)
  # Calculate bin width
  dataRange <- range(mean_bc$perimeter_mean)
  binWidth <- (max(dataRange) - min(dataRange)) / 9
  theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
  ...
  xLim <- range(pretty(range(mean_bc$perimeter_mean)))
  boxplot(perimeter_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
  ...
  myMatrix <- matrix(c(1,2), nrow = 2, ncol = 1)
    layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
    par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
  xyLim <- range(pretty(range(mean_bc$symmetry_mean)))
  benign <- mean_bc[mean_bc$diagnosis == 0, "symmetry_mean"]
  hist_benign <- hist(benign, plot = FALSE)
  
  # Calculate histogram for malignant cases
  malignant <- mean_bc[mean_bc$diagnosis == 1, "symmetry_mean"]
  hist_malignant <- hist(malignant, plot = FALSE)
  
  # Set y-axis limits
  yLim <- range(pretty(range(hist_benign$counts, hist_malignant$counts)))
  yLim <- c(yLim[1], yLim[2] * 1.)
  # Calculate bin width
  dataRange <- range(mean_bc$symmetry_mean)
  binWidth <- (max(dataRange) - min(dataRange)) / 9
  theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
  ...
  xLim <- range(pretty(range(mean_bc$symmetry_mean)))
  boxplot(symmetry_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
  ...
  mtext(pretty(xLim), line = .4 , side = 1, las = 1, at = pretty(xLim))
    ...
    layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
    par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
  xyLim <- range(pretty(range(mean_bc$texture_mean)))
  benign <- mean_bc[mean_bc$diagnosis == 0, "texture_mean"]
  hist_benign <- hist(benign, plot = FALSE)
  
  # Calculate histogram for malignant cases
  malignant <- mean_bc[mean_bc$diagnosis == 1, "texture_mean"]
  hist_malignant <- hist(malignant, plot = FALSE)
  ...
  # Calculate bin width
  dataRange <- range(mean_bc$texture_mean)
  binWidth <- (max(dataRange) - min(dataRange)) / 9
  theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
  ...
  xLim <- range(pretty(range(mean_bc$texture_mean)))
  boxplot(texture_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
  ...
  myMatrix <- matrix(c(1,2), nrow = 2, ncol = 1)
    layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
    par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
  xyLim <- range(pretty(range(mean_bc$compactness_mean)))
  benign <- mean_bc[mean_bc$diagnosis == 0, "compactness_mean"]
  hist_benign <- hist(benign, plot = FALSE)
  
  # Calculate histogram for malignant cases
  malignant <- mean_bc[mean_bc$diagnosis == 1, "compactness_mean"]
  hist_malignant <- hist(malignant, plot = FALSE)
  
  # Set y-axis limits
  yLim <- range(pretty(range(hist_benign$counts, hist_malignant$counts)))
  yLim <- c(yLim[1], yLim[2] * 2)
  # Calculate bin width
  dataRange <- range(mean_bc$compactness_mean)
  binWidth <- (max(dataRange) - min(dataRange)) / 9
  theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
  ...
  xLim <- range(pretty(range(mean_bc$compactness_mean)))
  boxplot(compactness_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
  ...
  #----------------------------------------------------------------FRACTAL DIMENSION  MEAN----------------------------------------------------------------------------- 
    ...
    layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
    par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
  xyLim <- range(pretty(range(mean_bc$fractal_dimension_mean)))
  benign <- mean_bc[mean_bc$diagnosis == 0, "fractal_dimension_mean"]
  hist_benign <- hist(benign, plot = FALSE)
  
  # Calculate histogram for malignant cases
  malignant <- mean_bc[mean_bc$diagnosis == 1, "fractal_dimension_mean"]
  hist_malignant <- hist(malignant, plot = FALSE)
  
  # Set y-axis limits
  yLim <- range(pretty(range(hist_benign$counts, hist_malignant$counts)))
  yLim <- c(yLim[1], yLim[2] * 1.15)
  # Calculate bin width
  dataRange <- range(mean_bc$fractal_dimension_mean)
  binWidth <- (max(dataRange) - min(dataRange)) / 9
  theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
  ...
  xLim <- range(pretty(range(mean_bc$fractal_dimension_mean)))
  boxplot(fractal_dimension_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
  ...
  myMatrix <- matrix(c(1,2), nrow = 2, ncol = 1)
    layout(myMatrix, widths = c(1,1) , heights = c(0.75,0.25) )
    par(mai = c(.2,1,0,.2), omi = c(.25,.25,.25,.25), lwd = 2)
  xyLim <- range(pretty(range(mean_bc$smoothness_mean)))
  benign <- mean_bc[mean_bc$diagnosis == 0, "smoothness_mean"]
  hist_benign <- hist(benign, plot = FALSE)
  
  # Calculate histogram for malignant cases
  malignant <- mean_bc[mean_bc$diagnosis == 1, "smoothness_mean"]
  hist_malignant <- hist(malignant, plot = FALSE)
  ...
  yLim <- c(yLim[1], yLim[2] * 1.25)
  # Calculate bin width
  dataRange <- range(mean_bc$smoothness_mean)
  binWidth <- (max(dataRange) - min(dataRange)) / 9
  theBreaks <- seq(min(dataRange), max(dataRange) + binWidth, binWidth)
  ...
  xLim <- range(pretty(range(mean_bc$smoothness_mean)))
  boxplot(smoothness_mean ~ diagnosis, data = mean_bc, ylim = xLim, axes = FALSE, ann = FALSE, col = "white", border = c(colors[4],colorsT[3]), lwd = 2, boxwex = .25, staplewex = .25, horizontal = TRUE)
